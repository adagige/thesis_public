...
@torch.no_grad()
def eval_epoch(logger, loader, model, split='val', return_pred=False):
    model.eval()
    ...
    while True:
        try:
            ...
            if cfg.gnn.head == 'inductive_edge':
             ...
             #   print('Batch is here\n\n', batch, flush=True)
              ...
              #      print('Edges were correcly returned:', edge_id.size(), flush=True)
               #     print('Shape pred:',pred.size(), 'Shape true:', true.size())
                else:
                    pred, true = model(batch)
                extra_stats = {}
        ...
        if return_pred:
            df.to_csv(f"{cfg.out_dir}/predictions.csv", index=False)
