...
class FeatureEncoder(torch.nn.Module):
    ...
    def __init__(self, dim_in, dataset):
        ...
        self.node_encoder = HeteroRawNodeEncoder(cfg.gt.dim_hidden, dataset)
       # self.node_encoder = NodeEncoder(64, dataset)
        ...
        self.edge_encoder = HeteroRawEdgeEncoder(cfg.gnn.dim_edge, dataset)
...
class GTModel(torch.nn.Module):
    def __init__(self, dim_in, dim_out, dataset):
        super().__init__()
        self.is_hetero  = isinstance(dataset[0], HeteroData)
      #  dim_in = cfg.share.dim_in # ADa Added this line instead of dim_in being an integer 
        if self.is_hetero:
        ...
        GNNHead         = HeteroGNNEdgeHead #the 1 was earlier dim_in
