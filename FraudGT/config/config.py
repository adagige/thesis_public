...
def set_cfg(cfg):
    ...
    ''' Extra stuff added by Ada'''
    cfg.gt = CN()
    cfg.dataset.task_entity = ('node', 'to', 'node')
    cfg.device = 'cpu'
    cfg.train.add_ego_id = True 
    cfg.dataset.task_type ='classification'
   # cfg.share.dim_in = CN()
    cfg.gt.dim_hidden =  64  # `gt.dim_hidden` must match `gnn.dim_inner`
    cfg.gt.layers_post_gt = 2 #used in hetero_edge
    #all below here are from the gt_config file
    cfg.gt.attn_heads = 2 #8 originalt sat lidt lavere for test
    ...
    cfg.gt.layers = 1 #originaly 3, for now just 1
    cfg.gt.attn_mask = 'Edge'
    cfg.gt.l2_norm = False

    #Some training specifics:
  #  cfg.wandb.use = False #Normally True
    cfg.optim.max_epoch = 500 #Would be 500 
    cfg.run_dir = '.\runs'
    cfg.optim.batch_accumulation = 8
...
# import fraudGT.graphgym.contrib.config
