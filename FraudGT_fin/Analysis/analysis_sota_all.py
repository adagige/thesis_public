...
def confusion_matrix_plot(data, title,save_to):
  ...
  data['predicted_binary'] = df_merged['Prediction']#df_merged['predicted_label']
#  return data
  ...
  plt.savefig(f'sota_results_all/{save_to}.png')
  plt.show()
  ...
  df.to_csv(f'sota_results_all/{save_to}.csv')
 # print(df)
...
def patterns_transactions(data, patterns, save_to ):
  ...
  #  patterns_reduced = patterns#[['Timestamp', 'From Bank', 'Account', 'To Bank', 'Account.1', 'Laundering Type','Is Laundering']]
    ...
    patterns['predicted_binary'] = df_merged['Prediction']#df_merged['predicted_label']
   ...
   # sns.heatmap(patterns,annot=True, ax=ax)
    ...
    patterns.to_csv(f'sota_results_all/{save_to}_patterns.csv')
    return 
...
all_banks = {'HI_Large':{'results_folder':'../.results/AML-Large-HI-SOTA-return-Predictions',
                          'new': 'AML-Large-HI-SOTA-return-Predictions',
              ...
              'LI_Large':{'results_folder':'../.results/AML-Large-LI-SOTA-return-Predictions',
                          'new': 'AML-Large-LI-SOTA-return-Predictions',
              ...
              'HI_Medium':{'results_folder':'../.results/AML-Medium-HI-SOTA-return-Predictions',
                            'new': 'AML-Medium-HI-SOTA-return-Predictions',
              ...
              'LI_Medium':{'results_folder':'../.results/AML-Medium-LI-SOTA-return-Predictions',
                            'new': 'AML-Medium-LI-SOTA-return-Predictions',
              ...
              'HI_Small':{'results_folder':'../.results/AML-Small-HI-SOTA-return-Predictions',
                            'new': 'AML-Small-HI-SOTA-return-Predictions',
              ...
              'LI_Small':{'results_folder':'../.results/AML-Small-LI-SOTA-return-Predictions',
                            'new':'AML-Small-LI-SOTA-return-Predictions',
...
for model in all_banks.values():
    ...
    if False: #os.path.exists(f'{results_folder}/predictions_all.csv'):
      df_merged = pd.read_csv(f'{results_folder}/predictions_all.csv')
      ...
      predictions_test_n = pd.read_csv(f'{results_folder}/{new_path}/predictions_test.csv',on_bad_lines='skip').drop_duplicates('Edge_ID')
      predictions_val_n = pd.read_csv(f'{results_folder}/{new_path}/predictions_val.csv',on_bad_lines='skip').drop_duplicates('Edge_ID')
      predictions_train_n = pd.read_csv(f'{results_folder}/{new_path}/predictions_train.csv',on_bad_lines='skip').drop_duplicates('Edge_ID')



   #   predictions = predictions_test
      # predictions = pd.concat([predictions_test,predictions_val,predictions_train,predictions_test_n,predictions_val_n,predictions_train_n])

      predictions = pd.concat([predictions_test_n,predictions_val_n,predictions_train_n])
      ...
      df_merged.to_csv(f'{results_folder}/predictions_all.csv', index=False)

    print(df_merged.columns, flush=True)

    df_merged.head(100).to_csv('sota_results_all/head_merged.csv')
    predictions_test = pd.read_csv(f'{results_folder}/{new_path}/predictions_test.csv').drop_duplicates('Edge_ID')
    predictions_test.head(100).to_csv('sota_results_all/head_test.csv')


    print('Predictions column size: ', len(predictions_test.columns), flush=True)
    print('Predictions head', predictions_test.head(10), flush=True)
    print('Predictions prediction', predictions_test['Prediction'].head(10), flush=True)
    # print(predictions_test.columns, flush=True)
