...
#  return data
  ...
  plt.savefig(f'auth_results/{save_to}.png')
  plt.show()
  ...
  df.to_csv(f'auth_results/{save_to}.csv')
 # print(df)
...
def patterns_transactions(data, patterns, save_to ):
  ...
  #  patterns_reduced = patterns#[['Timestamp', 'From Bank', 'Account', 'To Bank', 'Account.1', 'Laundering Type','Is Laundering']]
    ...
    data['Timestamp_x'] = data['Timestamp_x'].astype(object)

    # Merge the DataFrames
    merged_df = data.merge(
        patterns,  #
        how='left',  # Use 'left' to keep all rows from the first DataFrame
        left_on=['Timestamp_x','From Bank_x','To Bank_x', 'Account_x','Account.1_x','Amount Paid_x','Amount Received_x'],#[ 'From Bank', 'Account', 'To Bank', 'Account.1','Is Laundering_x'],  # Keys for the first DataFrame
    ...
    patterns.to_csv(f'auth_results/test_of_patterns.csv')
    patterns = patterns[['True_Label','predicted_binary','Laundering Type']]


    patterns['True_Label'] = 1

    patterns['True_Label'] = patterns['True_Label'].astype(int)
   ...
   # sns.heatmap(patterns,annot=True, ax=ax)
    ...
    patterns.to_csv(f'auth_results/{save_to}_patterns.csv')
    return 

BF_AF_HI = {'data': pd.read_csv('../bank_results/HI-Large_auth_FraudGT/AML_HI-Large_auth_BFGT_FraudGT/HI_BFGT_FraudGT_allpredictions_all.csv'), 'name': 'HI_BFGT_FraudGT', 'title': 'High bank FraudGT Authority FraudGT'}
BF_AF_LI = {'data': pd.read_csv('../bank_results/LI-Large_auth_FraudGT/AML_LI-Large_auth_BFGT_FraudGT/LI_BFGT_FraudGT_allpredictions_all.csv'),'name':'LI_BFGT_FraudGT', 'title': 'Low bank FraudGT Authority FraudGT'}

BF_AG_HI = {'data': pd.read_csv('../bank_results/HI-Large_auth_GCN/AML_HI-Large_auth_BFGT_GCN/HI_BFGT_GCN_allpredictions_all.csv'), 'name': 'HI_BFGT_GCN', 'title': 'High bank FraudGT Authority GatedGCN'}
BF_AG_LI = {'data': pd.read_csv('../bank_results/LI-Large_auth_GCN/AML_LI-Large_auth_BFGT_GCN/LI_BFGT_GCN_allpredictions_all.csv'), 'name': 'LI_BFGT_GCN','title': 'Low bank FraudGT Authority GatedGCN'}

BG_AG_HI = {'data': pd.read_csv('../bank_results_GatedGCN/HI-Large_auth_GCN/AML_HI-Large_auth_BGCN_GCN/HI_BGCN_GCN_allpredictions_all.csv'), 'name': 'HI_BGCN_GCN','title': 'High bank GatedGCN Authority GatedGCN'}
BG_AG_LI = {'data': pd.read_csv('../bank_results_GatedGCN/LI-Large_auth_GCN/AML_LI-Large_auth_BGCN_GCN/LI_BGCN_GCN_allpredictions_all.csv'), 'name': 'LI_BGCN_GCN','title': 'Low bank GatedGCN Authority GatedGCN'}

BG_AF_HI = {'data': pd.read_csv('../bank_results_GatedGCN/HI-Large_auth_FraudGT/AML_HI-Large_auth_BGCN_FraudGT/HI_BGCN_FraudGT_allpredictions_all.csv'), 'name': 'HI_BGCN_FraudGT','title': 'High bank GatedGCN Authority FraudGT'}
BG_AF_LI = {'data': pd.read_csv('../bank_results_GatedGCN/LI-Large_auth_FraudGT/AML_LI-Large_auth_BGCN_FraudGT/LI_BGCN_FraudGT_allpredictions_all.csv'), 'name': 'LI_BGCN_FraudGT','title': 'Low bank GatedGCN Authority FraudGT'}
...
for model in models_HI_list: 
   ...
   # print('Columns are named:',all_banks_HI.columns, flush=True)
    ...
    merge_shape = merged.shape

    merged = merged[merged['Prediction']!=10]

    print('Filtered these transactions:', merge_shape[0]-merged.shape[0], flush=True)

    merged = merged.drop_duplicates('EdgeID')

    # print(merged.columns, flush=True)
    # print(merged.dtypes, flush= True)

    patterns_transactions(merged, patterns_HI,model_name )
    ...
    table = merged.groupby(['True Label'])[['not received', 'predicted not-fraud', 'predicted fraud']].sum().T.rename(columns={0:'Not-Fraud', 1:'Fraud'})#.to_csv('sample_auth.csv')

    table.to_csv(f'auth_results/{model_name}.csv')

    # Calculate the total sum of all cells
    total = table.values.sum()  # or table.sum().sum()

    # Create an annotation array with the same shape as the table
    annot = np.empty(table.shape, dtype=object)
    for i in range(table.shape[0]):
        for j in range(table.shape[1]):
            count = table.iloc[i, j]  # use .iloc for positional indexing
            percentage = count / total * 100
            annot[i, j] = f'{count:,}\n({percentage:.1f}%)'  # add thousand separators for counts
...
#   sns.heatmap(conf_matrix, annot=annot, fmt='', cmap='PuRd', xticklabels=class_labels, yticklabels=class_labels)
    ...
    plt.figure(figsize=(6, 4))
    sns.heatmap(table, annot=annot, fmt='' , cmap='PuRd')#, xticklabels=class_labels, yticklabels=class_labels) fmt=",.0f"
    plt.yticks(rotation=0)
    plt.title(f'{model['title']}')
    plt.savefig(f'auth_results/{model_name}.png',bbox_inches = "tight")
    plt.close()
...
for model in models_LI_list: 
    ...
    patterns_transactions(merged, patterns_LI,model_name )
...
#   sns.heatmap(conf_matrix, annot=annot, fmt='', cmap='PuRd', xticklabels=class_labels, yticklabels=class_labels)
    ...
    plt.figure(figsize=(6, 4))
    sns.heatmap(table, annot=annot, fmt='' , cmap='PuRd')
    plt.yticks(rotation=0)
    plt.title(f'{model['title']}')
    plt.savefig(f'auth_results/{model_name}.png',bbox_inches = "tight")
