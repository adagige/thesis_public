...
def save_ckpt(
    ...
    if best:
        os.makedirs(get_ckpt_dir(best=True), exist_ok=True)
        torch.save(ckpt, get_ckpt_path(get_ckpt_epoch(epoch), best=True))
...
def get_ckpt_dir(best=False) -> str:
    if best:
        dir = osp.join(cfg.run_dir, 'ckpt/best')
    else:
        dir = osp.join(cfg.run_dir, 'ckpt')
    return dir


def get_ckpt_path(epoch: Union[int, str],best=False) -> str:
    return osp.join(get_ckpt_dir(best), f'{epoch}.ckpt')
