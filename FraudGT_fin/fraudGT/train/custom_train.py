...
@torch.no_grad()
def eval_epoch(logger, loader, model, split='val', return_pred=False, only_test = False):
    model.eval()
    pbar = tqdm(total=len(loader), disable=not cfg.val.tqdm)
    iterator = iter(loader)

    # Create a DataFrame
    df = pd.DataFrame(columns=["Edge_ID", "Prediction", "Prediction_prob", "True_Label"])
    #Create a data file 
    if not os.path.exists(f"{cfg.out_dir}/predictions_{split}.csv"):
        df.to_csv(f"{cfg.out_dir}/predictions_{split}.csv", index=False)
    
    if only_test and not os.path.exists(f"{cfg.out_dir}/predictions_only_{split}.csv"):
        df.to_csv(f"{cfg.out_dir}/predictions_only_{split}.csv", index=False)
    ...
    while True:
        try:
            ...
            if cfg.gnn.head == 'inductive_edge':
             ...
             #   print('Batch is here\n\n', batch, flush=True)
                if return_pred:
                    pred, true, edge_id = model(batch, return_edges=True,only_test=True)
          
                else:
                    pred, true = model(batch)
                extra_stats = {}
                # print('Number of unique nodes in batch:',batch[('node', 'to', 'node')].edge_label_index.unique().size(),flush=True)
            
                # print('\n\nPrediction shape out of model\n\n', pred.size(),'True shape out of model\n\n', true.size(),flush=True)
                # print("Number of edges after filtering:", batch[('node', 'to', 'node')].split_mask.sum(), flush=True)
            ...
            #    pred = _pred.squeeze(-1) if _pred.ndim > 1 else _pred
             #   true = _true.squeeze(-1) if _true.ndim > 1 else _true
                batch_df = pd.DataFrame({
                        "Edge_ID": edge_id.cpu().numpy(),
                        "Prediction": pred_int.numpy(),
                        "Prediction_prob": pred_score.numpy(),
             ...
             #   df = pd.concat([df, batch_df], ignore_index=True)
                if only_test:
                    batch_df.to_csv(f"{cfg.out_dir}/predictions_only_{split}.csv",mode='a', index=False,header=False )
                else:
                    batch_df.to_csv(f"{cfg.out_dir}/predictions_{split}.csv",mode='a', index=False,header=False )
              #  print('\n\n Predictions', pred, '\n true :\n ',true, flush = True )
            #    classified_1 = _pred > 0.5  # Change threshold if needed
              #  print('Prediction shape', _pred.size(),flush=True)
                # Get indices of instances classified as 1
               # print('Batch is here', batch, flush=True)
               # print("Classified as 1 mask shape:", classified_1.shape,flush=True)
                #print("Edge ID shape:", batch[('node', 'to', 'node')].edge_id.shape,flush=True)
                #print("Filtered indices shape:", classified_1.nonzero().squeeze().shape,flush=True)

               # identified_inputs = batch[('node', 'to', 'node')].edge_label_index[:,classified_1]  # Adjust this depending on your batch structure
              #  return pred, true
                logger.update_stats(true=_true,
                    pred=_pred,
                    loss=loss.detach().cpu().item(),
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        if cfg.train.enable_ckpt and is_ckpt_epoch(cur_epoch):
            #and not cfg.train.ckpt_best \ #THis is removed by adgi
            save_ckpt(model, optimizer, scheduler, cur_epoch, best=False)
        ...
        if is_eval_epoch(cur_epoch, start_epoch):
            ...
            if cfg.train.enable_ckpt and cfg.train.ckpt_best and \
                ...
                save_ckpt(model, optimizer, scheduler, cur_epoch, best=True)
