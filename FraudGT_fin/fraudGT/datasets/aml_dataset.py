...
def format_dataset(inPath):
    ...
    '''
    print('Loading the dataset with aml_dataset')
...
class AMLDataset(TemporalDataset):
    ...
    def add_ports_func(self, data, ports):
        reverse_ports = True
        if not self.reverse_mp: #if true then False
    ...
    def process(self):
        ...
        edge_id = torch.Tensor(df_edges['EdgeID'].to_numpy()) ### ADDED ADA
        ...
        for split in ['train', 'val', 'test']:
            ...
            masked_edgeid = edge_id[e_mask] #added ADA
            ...
            data['node', 'to', 'node'].edge_id = masked_edgeid #ADDED by ADA
            # if args.ports:
