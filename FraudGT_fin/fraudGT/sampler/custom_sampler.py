...
@register_sampler('link_neighbor_all')
def get_LinkNeighborAllLoader(dataset, batch_size, shuffle=False, split='train'):
    '''New sampler added by Ada to be able to get predictions on all transactions 
    Used for bank to authority pipeline'''
    task = cfg.dataset.task_entity
    data = dataset[split]
    data = dataset[split]
    mask = data[task].split_mask
    ...
    # For evaluation, we use all available edges.
    # edge_label_index = data[task].edge_index  # use full edge index without masking
    # edge_label = data[task].y  # include all edge labels

    total_edges = edge_label_index.size(1)
    iter_per_epoch = (total_edges + batch_size - 1) // batch_size

    # Use evaluation neighbor sizes if provided, otherwise default back to training neighbor sizes.
    num_neighbors = cfg.eval.neighbor_sizes if hasattr(cfg, 'eval') else cfg.train.neighbor_sizes

    loader_eval = LoaderWrapper(
        LinkNeighborLoader(
            data=data,
        ...
        iter_per_epoch, #getattr(cfg, 'val' if split == 'test' else split).iter_per_epoch,
        split
    )
